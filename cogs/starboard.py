...
class StarboardCog(commands.Cog):
    ...
    @commands.hybrid_group(name="starboard", description="Gestionar la configuración del Starboard")
    @app_commands.default_permissions(administrator=True)
    async def starboard(self, ctx: commands.Context):
        if ctx.invoked_subcommand is None:
            await ctx.send("Usa `!starboard setup` o `!starboard config`", ephemeral=True)

    @starboard.command(name="setup", description="Configura o actualiza el Starboard")
    @app_commands.describe(channel="El canal donde se publicarán los mensajes destacados", threshold="Mínimo de estrellas requeridas (por defecto: 4)")
    async def setup(self, ctx: commands.Context, channel: discord.TextChannel, threshold: int = 4):
        if threshold < 1:
            return await ctx.send("❌ El umbral debe ser al menos 1.", ephemeral=True)
        ...
        await ctx.send(f"✅ Starboard configurado en {channel.mention} con un umbral de **{threshold}** estrellas.")

    @starboard.command(name="config", description="Muestra la configuración actual del Starboard")
    async def config(self, ctx: commands.Context):
        config = starboard_col.find_one({"guild_id": ctx.guild.id})
        if not config:
            return await ctx.send("❌ El Starboard no está configurado para este servidor.")
        
        channel = self.bot.get_channel(config["channel_id"])
        channel_name = channel.mention if channel else "Desconocido/Eliminado"
        await ctx.send(f"📋 **Configuración del Starboard:**\n- **Canal:** {channel_name}\n- **Umbral:** {config['threshold']} ⭐")
    ...
    def create_starboard_embed(self, message: discord.Message, star_count: int):
        ...
        embed.add_field(name="Original", value=f"[¡Ir al mensaje!]({message.jump_url})")
        ...
        if star_count >= threshold:
            ...
            if starboard_entry:
                try:
                    ...
                    # El mensaje fue eliminado del starboard, volver a publicarlo
            ...
            # Si ya estaba publicado pero las estrellas bajaron del umbral (poco probable solo agregando, pero útil para eliminación)
            # El requisito del usuario dice "alcanza un umbral", no indica qué hacer si baja.
            # Por lo general se queda o se elimina. Se deja así por ahora a menos que se implemente la eliminación.
            pass
