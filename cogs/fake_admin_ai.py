...
class FakeAdminAI(commands.Cog):
    def __init__(self, bot):
        ...
        if not api_key:
            print("⚠️ GROQ_API_KEY no encontrado. FakeAdminAI no funcionará.")
        ...
        if channel_id_raw:
            try:
                ...
                print(f"⚠️ AI_CHANNEL_ID inválido: {channel_id_raw}. FakeAdminAI no funcionará.")
                self.channel_id = 0
        else:
            print("⚠️ AI_CHANNEL_ID no encontrado. FakeAdminAI no funcionará.")
            self.channel_id = 0
