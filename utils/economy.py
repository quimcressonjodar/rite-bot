...
class JailCheckError(commands.CheckFailure):
    """Lanzada por la verificación global de cárcel para poder suprimirla de forma diferenciada."""
    pass


def get_user_data(user_id: str) -> dict:
    """Obtiene los datos económicos de un usuario; los crea si no existen."""
    ...
    if "balance" in user:
        # Migrar campo 'balance' antiguo al nuevo campo 'wallet'
...
def parse_economy_amount(amount_input: str, max_balance: int) -> int:
    """Convierte una entrada de cantidad ('all', 'half' o número) a un entero.

    Devuelve -1 si el valor no es válido.
    """
...
JAIL_DURATION = 5400  # 90 minutos en segundos


def set_jail(user_id: str, duration: int = JAIL_DURATION) -> int:
    """Mete a un usuario en la cárcel durante `duration` segundos. Devuelve el timestamp de liberación."""
...
def is_jailed(user_id: str) -> int:
    """Devuelve el timestamp de liberación si el usuario está actualmente en la cárcel, de lo contrario 0."""
